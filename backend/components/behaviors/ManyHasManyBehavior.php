...
use yii\base\Behavior as Behavior;
...
class ManyHasManyBehavior extends Behavior {
    ...
    {
        return [
            \yii\db\ActiveRecord::EVENT_AFTER_INSERT => 'changeRelations',
            \yii\db\ActiveRecord::EVENT_AFTER_UPDATE => 'changeRelations',
            \yii\db\ActiveRecord::EVENT_BEFORE_DELETE => 'changeRelations',
        ];
    }
            ...
            if (!empty($relation->via) && $relation->multiple) {                
                ...
                try {                    
                    if (!empty($this->_values[$attributeName])) {
                        ...
                        $oldValues = \yii\helpers\ArrayHelper::getColumn(\yii\helpers\ArrayHelper::toArray($this->owner->$relationName), $relationPrimaryColumn);                        
